...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom {
	...
	private BooleanExpression searchByLike(String searchBy, String searchQuery) {
		if(StringUtils.equals("itemNm", searchBy)) {
			...
			return QItem.item.createdBy.like("%" + searchQuery + "%"); // createBy LIKE %test.com%
		}
		return null;
	}
	...
	public Page<Item> getAdminItemPage(ItemSearchDto itemSearchDto, Pageable pageable) {
		...
		List<Item> content = queryFactory.selectFrom(QItem.item) // select * from item
				.where(regDtsAfter(itemSearchDto.getSearchDateType()), // where reg_item = Id 
		...
		long total = content.size(); // 전체 레코드 개수
